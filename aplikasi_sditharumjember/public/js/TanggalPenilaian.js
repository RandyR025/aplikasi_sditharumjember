...
$(document).on('click', '.edit_penilaian', function(e){
   ...
   $.ajax({
    type: "GET",
    url: "/aplikasi_sditharumjember/edit-tanggalpenilaian/"+penilaian_id,
    success: function(response){
      console.log(response);
      if(response.status == 404){
        ...
        $("#edit_idpenilaian").val(response.tanggal[0].id_penilaian).trigger('change');
        $('#edit_tanggalpelaksanaan').val(response.tanggal[0].tanggal);
        $('#edit_deadline').val(response.tanggal[0].deadline);
      }
    }
   });
  });
...
$(document).on("submit", "#penilaian_form", function (e) {
  ...
  $.ajax({
      type: "POST",
      url: "/aplikasi_sditharumjember/update-tanggalpenilaian/" + penilaian_id,
      data: EditformData,
      contentType: false,
      processData: false,
      beforeSend: function () {
...
$(document).on("click", ".delete_penilaian", function (e) {
  ...
  swalWithBootstrapButtons.fire({
    ...
    $.ajax({
        type: "DELETE",
        url: "/aplikasi_sditharumjember/delete-tanggalpenilaian/" + penilaian_id,
        success: function (response) {
            console.log(response);
            // $("#success_message").addClass("alert alert-success");
            // $("#success_message").text(response.message);
