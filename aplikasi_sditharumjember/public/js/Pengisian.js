...
// }
 ...
 $(document).on('click', '.edit_pengisian', function(e){
   ...
   $.ajax({
    type: "GET",
    url: "/aplikasi_sditharumjember/edit-pengisian/"+penilaian_id,
    success: function(response){
      ...
      if(response.status == 404){
        ...
        $.ajax({
          url: '/aplikasi_sditharumjember/getsubkriteria/'+response.pengisian[0].kode_kriteria,
          type: "GET",
          dataType: "json",
  ...
  $(document).on("submit", "#pengisian_form", function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "/aplikasi_sditharumjember/update-pengisian/" + penilaian_id,
        data: EditformData,
        contentType: false,
        processData: false,
        beforeSend: function () {
  ...
  $(document).on("click", ".delete_pengisian", function (e) {
    ...
    swalWithBootstrapButtons.fire({
      ...
      $.ajax({
          type: "DELETE",
          url: "/aplikasi_sditharumjember/delete-pengisian/" + pengisian_id,
          success: function (response) {
              console.log(response);
              // $("#success_message").addClass("alert alert-success");
              // $("#success_message").text(response.message);
  ...
  $(document).ready(function() {
    $('#kode_kriteria').on('change', function() {
       ...
       if(kriteriaID) {
           $.ajax({
               url: '/aplikasi_sditharumjember/getsubkriteria/'+kriteriaID,
               type: "GET",
               dataType: "json",
               success:function(response)
               {
