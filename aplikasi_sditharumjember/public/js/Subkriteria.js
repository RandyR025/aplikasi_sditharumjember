...
// }
 ...
 $.ajax({
  type: "GET",
  url: "/aplikasi_sditharumjember/edit-subkriteria/"+kriteria_id,
  success: function(response){
...
$(document).on("submit", "#subkriteria_form", function (e) {
  ...
  $.ajax({
      type: "POST",
      url: "/aplikasi_sditharumjember/update-subkriteria/" + kriteria_id,
      data: EditformData,
      contentType: false,
      processData: false,
      beforeSend: function () {
...
$(document).on("click", ".delete_subkriteria", function (e) {
  ...
  swalWithBootstrapButtons.fire({
    ...
    $.ajax({
        type: "DELETE",
        url: "/aplikasi_sditharumjember/delete-subkriteria/" + subkriteria_id,
        success: function (response) {
            window.location.reload();
            console.log(response);
            // $("#success_message").addClass("alert alert-success");
